...
const hasPurgeCssPrerequisites =
  DECLARED_BUNX_TARGETS.has("purgecss") &&
  existsSync("src/app/globals.css") &&
  existsSync("src/components/components.css");
const hasTsdPrerequisites =
  DECLARED_BUNX_TARGETS.has("tsd") &&
  existsSync("next-env.d.ts") &&
  existsSync("next-env.test-d.ts");
...
const architecture = defineStep({
  data: {
    ...
    rules: {
      ...
      "junk-drawer-file": {
        fileNamePatterns: ["*helper*", "*runtime*", "*util*", "*support*"],
      },
...
const purgeCss = defineStep({
  ...
  enabled: hasPurgeCssPrerequisites,
